...
def init_db():
    with sqlite3.connect('farm.db', timeout=20) as conn:
        ...
        # Ежедневный бонус - пересоздаём таблицу
        try:
        ...
        if admin_count == 0:
            ...
            try:
                conn.execute('''
                    INSERT INTO users (login, password, balance, grid_size, storage_level, created_at, is_admin) 
                    VALUES (?, ?, 100000, 3, 1, ?, 1)
...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect('farm.db', timeout=20)
...
def get_user_with_stats(user_id, skip_harvest=False):
    try:
        ...
        conn = get_db()
        user = conn.execute('SELECT login, balance, grid_size, storage_level FROM users WHERE id = ?', (user_id,)).fetchone()
        if not user:
            return None
        user_dict = dict(user)
...
def check_ton_transactions():
    try:
        conn = sqlite3.connect('farm.db', timeout=20)
...
def login():
    if request.method == 'POST':
        ...
        if action == 'login':
            ...
            cursor = conn.execute('INSERT INTO users (login, password, balance, grid_size, storage_level, referrer_id, created_at, is_admin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
def plant(cell_id):
    print("=" * 50)
    print("PLANT FUNCTION CALLED")
    print(f"Cell ID: {cell_id}")
    
    crop_key = request.form.get('crop')
    print(f"Crop key: {crop_key}")
    ...
    print(f"User balance: {user['balance']}, Crop cost: {crop_cost}")
    ...
    if user['balance'] >= crop_cost:
        ...
        # Исправленный запрос - используем INSERT OR REPLACE
        ...
        check = conn.execute('SELECT crop FROM garden WHERE user_id = ? AND cell_id = ?', 
                            (session['user_id'], cell_id)).fetchone()
        print(f"After save - crop in DB: {check['crop'] if check else 'None'}")
        ...
        print(f"SUCCESS: Planted {crop_name}")
    else:
...
def expand_garden():
    ...
    if user['balance'] >= expand_cost:
        ...
        conn.execute('UPDATE users SET balance = ?, grid_size = ? WHERE id = ?', (new_balance, new_size, session['user_id']))
...
def sell_crop(crop):
    ...
    conn.execute('UPDATE users SET balance = ? WHERE id = ?', (new_balance, session['user_id']))
    conn.commit()
    flash(f'💰 Продано {quantity} {VEGETABLES[crop]["name"]} за {total_earned} Coin', 'success')
    return redirect(url_for('storage'))
...
def sell_all_crop(crop):
    ...
    conn.execute('UPDATE users SET balance = ? WHERE id = ?', (new_balance, session['user_id']))
    conn.commit()
    flash(f'💰 Продано всё {VEGETABLES[crop]["name"]} ({quantity} шт) за {total_earned} Coin', 'success')
    return redirect(url_for('storage'))
...
def sell_all_storage():
    ...
    conn.commit()
    flash(f'💰 Продано всё ({total_items} шт) за {total_earned} Coin', 'success')
    return redirect(url_for('storage'))
...
def upgrade_storage():
    ...
    if user['balance'] >= upgrade_cost:
        new_balance = user['balance'] - upgrade_cost
        conn.execute('UPDATE users SET balance = ?, storage_level = ? WHERE id = ?', (new_balance, current_level + 1, session['user_id']))
        conn.commit()
...
def referrals():
    ...
    if not user:
        flash('❌ Ошибка загрузки данных', 'error')
        return redirect(url_for('logout'))
    referral_link = f"http://localhost:5000/login?ref={user['login']}"
...
def claim_daily_bonus():
    import traceback
    try:
        print("=== НАЧАЛО claim_daily_bonus ===")
        ...
        # Проверяем таблицу
        table_check = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='daily_bonus'").fetchone()
        print(f"Table daily_bonus exists: {table_check is not None}")
        
        if table_check:
            # Проверяем структуру таблицы
            columns = conn.execute("PRAGMA table_info(daily_bonus)").fetchall()
            print(f"Columns: {[c[1] for c in columns]}")
        
        # Проверяем существующую запись
        bonus_data = conn.execute('SELECT last_claim_date, streak, last_claim_time FROM daily_bonus WHERE user_id = ?', (user_id,)).fetchone()
        print(f"Bonus data: {bonus_data}")
        ...
        if not bonus_data:
            ...
            last_claim_date = bonus_data['last_claim_date']
            streak = bonus_data['streak'] if bonus_data['streak'] is not None else 0
            print(f"Last claim: {last_claim_date}, streak: {streak}")
            
            if last_claim_date == today:
                print("Бонус уже получен сегодня")
            ...
            if last_claim_date:
                try:
                    ...
                    if last_date == today_date - timedelta(days=1):
                        new_streak = min(streak + 1, 7)
                        bonus_amount = new_streak
                        print(f"Серия продолжается: {new_streak}")
                    else:
                        new_streak = 1
                        bonus_amount = 1
            ...
            conn.execute('UPDATE daily_bonus SET last_claim_date = ?, streak = ?, last_claim_time = ? WHERE user_id = ?',
                         (today, new_streak, time.time(), user_id))
            conn.commit()
            print(f"Обновлено: streak={new_streak}, bonus={bonus_amount}")
        
        # Начисляем бонус
        conn.execute('UPDATE users SET balance = balance + ? WHERE id = ?', (bonus_amount, user_id))
        conn.commit()
        print(f"Начислено {bonus_amount} Coin")
...
def admin_withdraws():
    conn = get_db()
    withdraws = conn.execute('SELECT w.*, u.login, u.created_at, u.balance FROM withdraw_requests w JOIN users u ON w.user_id = u.id ORDER BY w.created_at DESC').fetchall()
...
def admin_users():
    conn = get_db()
    users = conn.execute('SELECT id, login, balance, storage_level, grid_size, created_at, is_admin FROM users ORDER BY balance DESC').fetchall()
...
def confirm_deposit(deposit_id):
    conn = get_db()
    deposit = conn.execute('SELECT user_id, amount FROM deposit_requests WHERE id = ? AND status = "pending"', (deposit_id,)).fetchone()
    if not deposit:
        flash('❌ Заявка не найдена', 'error')
    ...
    conn.execute('UPDATE deposit_requests SET status = "confirmed", confirmed_at = ?, txid = "manual_confirm" WHERE id = ?', (time.time(), deposit_id))
    conn.commit()
...
def ad_faucet():
    ...
    conn = get_db()
    ad_data = conn.execute('SELECT last_claim, daily_count, last_reset, ad_start_time FROM ad_rewards WHERE user_id = ?', 
    ...
    if ad_data:
        ...
        if now - last_reset >= 86400:
            ...
            daily_left = 3 - ad_data['daily_count']
        
        ad_start_time = ad_data['ad_start_time'] or 0
        if ad_start_time > 0:
            elapsed = now - ad_start_time
            if elapsed >= 10:
                ad_ready = True
        else:
            ad_ready = False
    ...
    return render_template('ad_faucet.html', 
                          user=user, 
                          can_claim=can_claim,
                          ad_ready=ad_ready,
...
def start_ad_watch():
    ...
    conn.execute('''INSERT OR REPLACE INTO ad_rewards (user_id, last_claim, daily_count, last_reset, ad_start_time) 
                    VALUES (?, ?, ?, ?, ?)''',
...
if __name__ == '__main__':
    ...
    print("="*50 + "\n")
    print("🔑 Для получения прав администратора:")
    print("   1. Зарегистрируйтесь/войдите в аккаунт")
    print("   2. Перейдите по ссылке: http://localhost:5000/make_me_admin")
    print("   3. ПОСЛЕ ЭТОГО УДАЛИТЕ ЭТОТ МАРШРУТ ИЗ ФАЙЛА app.py!")
    print("="*50 + "\n")
